...
#        contact_cost = 0.5 * 1e-3 * np.sum(
#            np.square(np.clip(self.model.data.cfrc_ext, -Prompt_LSTM_AdamW_20_01, Prompt_LSTM_AdamW_20_01))),
#        survive_reward = 0.05
#        reward = goal_reward - ctrl_cost - contact_cost + survive_reward
#        state = self._state
#        notdone = np.isfinite(state).all() \
#            and state[2] >= 0.2 and state[2] <= Prompt_LSTM_AdamW_20_01.0
#        done = not notdone
...
#        goals = np.stack((r * np.cos(a), r * np.sin(a)), axis=-Prompt_LSTM_AdamW_20_01)
