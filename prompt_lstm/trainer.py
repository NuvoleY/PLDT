...
class Trainer:
    ...
    def train_iteration(self, num_steps, no_prompt=False):
        ...
        for _ in range(num_steps):
            train_loss = self.train_steps(no_prompt)
    ...
    def train_steps(self, no_prompt=False):
    ...
    def eval_iteration(self, get_prompt, prompt_trajectories, eval_episodes, env_name, info,
                                variant, env, iter_num=0, print_logs=False, no_prompt=False, group='test'):
        print('evaluate at tasks: ', env_name)
        logs = dict()
        ...
        self.eval_fns = [eval_episodes(tar, info[env_name], variant, env, env_name) for tar in info[env_name]['env_targets']]
        self.get_prompt = get_prompt(prompt_trajectories, info[env_name], variant)
        if not no_prompt:
