...
def gen_env(env_name):
    if 'cheetah_dir' in env_name:
        ...
        max_ep_len = 1000
        env_targets = [13000]  # evaluation conditioning targets
        scale = 1000.  # normalization for rewards/returns
    elif env_name == 'door':
        ...
        env = gym.make('Peng-v0')
        max_ep_len = 1000
        env_targets = [1500]  # evaluation conditioning targets
        scale = 1000.  # normalization for rewards/returns
...
def get_prompt(prompt_trajectories, info, variant):
    ...
    def fn(sample_size=1):
        ...
        for i in range(int(sample_size)):
            # traj = prompt_trajectories[int(batch_inds[i])]  # Random selection of trajectories
            traj = prompt_trajectories[int(sorted_inds[-i])]  # Choose the trajectory with the highest return
            si = max(0, traj['rewards'].shape[0] - max_len-1)  # Select the last trajectory of length maxlen
...
def load_data_prompt(env_name, data_save_path, args):
    ...
    # select the path of the prompt trajectory
